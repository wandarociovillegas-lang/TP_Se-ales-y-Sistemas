...
def sec_mas_cercana(numero, secuencia):
    indices_secuencia = len(secuencia)-1
    return min(range(1, indices_secuencia), key=lambda x :abs(secuencia[x]-numero)) 
...
def psola(secuencia_vieja, periodos_viejos, nuevos_t0, signal_x, signal_y):
    ...
    for t in range(1,len(nuevos_t0)-1) : # recorro los t0 nuevos
        puntero = nuevos_t0[t]
        punto_seleccionado = sec_mas_cercana(puntero, secuencia_vieja) # me da el indice del pico viejo mas cercano al nuevo
